...
def train():
    ...
    with open("data/temp/py_1_word.txt", "r", encoding="gbk") as f:
        one_word_data = json.load(f)

    with open("data/temp/py_2_word.txt", "r", encoding="gbk") as f:
        two_word_data = json.load(f)
        for py_py in two_word_data:
            first_pinyin = py_py.split()[0]
            for key, value in two_word_data[py_py].items():
                first_word = key.split()[0]
                two_word_data[py_py][key] = math.log(
                    float(value) / one_word_data[first_pinyin][first_word]
                )
...
def viterbi(query_list, one_word_data, two_word_data):
    ...
    for i in range(len(query_list)):
        total_count = sum(
            [
                one_word_data[query_list[i]].get(wd, 0)
        ...
        layers.append(
            [
                Node(
                    ...
                    weight=(
                        -math.inf
                        if one_word_data[query_list[i]].get(wd, 0) == 0
                        else math.log(
                            float(one_word_data[query_list[i]].get(wd, 0)) / total_count
                        )
                    ),
                )
                for wd in pinyindict[query_list[i]]
    ...
    for i in range(1, len(layers)):
        pinyins = query_list[i - 1] + " " + query_list[i]
        for node in layers[i]:
            best_weight = -math.inf
            for pred in layers[i - 1]:
                connect = pred.word + " " + node.word
                new_weight = two_word_data.get(pinyins, None)
                if new_weight is not None:
                    new_weight = new_weight.get(connect, -30)
                else:
                    new_weight = -30
